      ...
      <tbody>
        @foreach($nama as $nam)
          <tr>
              ...
              <td><img src="{{ asset('/images/pengetahuan/'.$nam['image'])  }}" style="max-height:100px;max-width:100px;margin-top:5px;"></td>
              <td>{{ $nam['updated_at'] }}</td>
              <td>
                {!! Form::open(['route' => ['pDelete', $nam['id']], 'method' => 'DELETE']) !!}
                <a href="{{ route('plihat', $nam['id']) }}" class="btn btn-primary btn-sm">View</a>
                <a href="{{ route('phalEdit', $nam['id']) }}" class="btn btn-success btn-sm">Edit</a>
